...
// Upstream Version: 2.8.29
// https://github.com/serbanghita/Mobile-Detect/blob/2.8.29/Mobile_Detect.php
...
var (
	phoneDevices = [...]string{
		//IPHONE:
		`\biPhone\b|\biPod\b`, // |\biTunes
		//BLACKBERRY:
		`BlackBerry|\bBB10\b|rim[0-9]+`,
		//HTC:
		...
		//PALM:
		`PalmSource|Palm`, // avantgo|blazer|elaine|hiptop|plucker|xiino ; @todo - complete the regex.
		//VERTU:
		`Vertu|Vertu.*Ltd|Vertu.*Ascent|Vertu.*Ayxta|Vertu.*Constellation(F|Quest)?|Vertu.*Monika|Vertu.*Signature`, // Just for fun ;)
		...
		//WIKO:
		`KITE 4G|HIGHWAY|GETAWAY|STAIRWAY|DARKSIDE|DARKFULL|DARKNIGHT|DARKMOON|SLIDE|WAX 4G|RAINBOW|BLOOM|SUNSET|LENNY|BARRY|IGGY|OZZY|CINK FIVE|CINK PEAX|CINK PEAX 2|CINK SLIM|CINK SLIM 2|CINK +|CINK KING|CINK PEAX|CINK SLIM|SUBLIM`,
		//IMOBILE:
		...
		// @ref http://www.wolfgangmobile.com/
		//`Wolfgang`:
	...
	tabletDevices = [...]string{
		//IPAD:
		`iPad|iPad.*Mobile`, // @todo: check for mobile friendly emails topic.
		//NEXUSTABLET: `Android.*Nexus[\s]+(7|10)|^.*Android.*Nexus(?:(?!Mobile).)*$`,
		`Android.*Nexus[\s]+(7|9|10)`,
		//SAMSUNGTABLET: SCH-P709|SCH-P729|SM-T2558 - Samsung Mega - treat them like a regular phone.
		...
		//KINDLE:
		`Kindle|Silk.*Accelerated|Android.*\b(KFOT|KFTT|KFJWI|KFJWA|KFOTE|KFSOWI|KFTHWI|KFTHWA|KFAPWI|KFAPWA|WFJWAE|KFSAWA|KFSAWI|KFASWI|KFARWI|KFFOWI|KFGIWI|KFMEWI)\b`,
		...
		// @ref http://www.asus.com/de/Tablets_Mobile/Memo_Pad_Products/
		//ASUSTABLET:
		`^.*PadFone.*$|Transformer|TF101|TF101G|TF300T|TF300TG|TF300TL|TF700T|TF700KL|TF701T|TF810C|ME171|ME301T|ME302C|ME371MG|ME370T|ME372MG|ME172V|ME173X|ME400C|Slider SL101|\bK00F\b|\bK00C\b|\bK00E\b|\bK00L\b|TX201LA|ME176C|ME102A|\bM80TA\b|ME372CL|ME560CG|ME372CG|ME302KL| K010 | K011 | K017 | K01E |ME572C|ME103K|ME170C|ME171C|\bME70C\b|ME581C|ME581CL|ME8510C|ME181C|P01Y|PO1MA|P01Z|\bP027\b`,
		//BLACKBERRYTABLET:
		`PlayBook|RIM Tablet`,
		//HTCTABLET:
		...
		//LENOVOTABLET:
		`Lenovo TAB|Idea(Tab|Pad)( A1|A10| K1|)|ThinkPad([ ]+)?Tablet|YT3-850M|YT3-X90L|YT3-X90F|YT3-X90X|Lenovo.*(S2109|S2110|S5000|S6000|K3011|A3000|A3500|A1000|A2107|A2109|A1107|A5500|A7600|B6000|B8000|B8080)(-|)(FL|F|HV|H|)|TB-X103F|TB-X304F`,
		//DELLTABLET:
		...
		//PHILIPSTABLET:
		// @ref: http://www.support.philips.com/support/catalog/worldproducts.jsp?userLanguage=en&userCountry=cn&categoryid=3G_LTE_TABLET_SU_CN_CARE&title=3G%20tablets%20/%20LTE%20range&_dyncharset=UTF-8
		...
		//GUTABLET:
		`TX-A1301|TX-M9002|Q702|kf026`, // A12R|D75A|D77|D79|R83|A95|A106C|R15|A75|A76|D71|D72|R71|R73|R77|D82|R85|D92|A97|D92|R91|A10F|A77F|W71F|A78F|W78F|W81F|A97F|W91F|W97F|R16G|C72|C73E|K72|K73|R96G
		...
		//@ref http://www.intracon.eu/tablet
		//FX2TABLET:
		`FX2 PAD7|FX2 PAD10`,
		...
		// @ref: http://www1.viewsonic.com/products/computing/tablets/
		//VIEWSONICTABLET:
		...
		// @ref: http://www.breezetablet.com/
		//AOCTABLET:
		`MW0811|MW0812|MW0922|MTK8382|MW1031|MW0831|MW0821|MW0931|MW0712`,
		// @ref: http://www.mpmaneurope.com/en/products/internet-tablets-14/android-tablets-14/
		//MpmanTablet:
		...
		//UBISLATETABLET:
		`UbiSlate[\s]?7C`,
		// @ref: http://www.pocketbook-int.com/ru/support
		// PocketBookTablet
		`Pocketbook`,
		// @ref: http://www.kocaso.com/product_tablet.html
		//KocasoTablet:
		`\b(TB-1207)\b`,
		// @ref: http://global.hisense.com/product/asia/tablet/Sero7/201412/t20141215_91832.htm
		//HisenseTablet:
		`\b(F5281|E2371)\b`,
		...
		`T-Hub2`,
		//GenericTablet
		//@full, notice Tablet(?!.*PC) - `Android.*\b97D\b|Tablet(?!.*PC)|BNTV250A|MID-WCDMA|LogicPD Zoom2|\bA7EB\b|CatNova8|A1_07|CT704|CT1002|\bM721\b|rk30sdk|\bEVOTAB\b|M758A|ET904|ALUMIUM10|Smartfren Tab|Endeavour 1010|Tablet-PC-4|Tagi Tab|\bM6pro\b|CT1020W|arc 10HD|\bJolla\b|\bTP750\b
		`Android.*\b97D\b|BNTV250A|MID-WCDMA|LogicPD Zoom2|\bA7EB\b|CatNova8|A1_07|CT704|CT1002|\bM721\b|rk30sdk|\bEVOTAB\b|M758A|ET904|ALUMIUM10|Smartfren Tab|Endeavour 1010|Tablet-PC-4|Tagi Tab|\bM6pro\b|CT1020W|arc 10HD|\bTP750\b|\bQTAQZ3\b`,
	}
	operatingSystems = [...]string{
		...
		//WINDOWSPHONEOS:
		//@ref http://msdn.microsoft.com/library/ms537503.aspx
...
func (r *Rules) setMobileDetectionRules(nameToKey map[string]int) {
	...
	j := 0
	count := len(r.phoneDevices)
	for i := 0; i < count; i++ {
		combined[j] = r.phoneDevices[i]
		j += 1
	}

	count = len(r.tabletDevices)
	for i := 0; i < count; i++ {
		combined[j] = r.tabletDevices[i]
		j += 1
	}

	count = len(r.operatingSystems)
	for i := 0; i < count; i++ {
		combined[j] = r.operatingSystems[i]
		j += 1
	}

	count = len(r.browsers)
	for i := 0; i < count; i++ {
		combined[j] = r.browsers[i]
		j += 1
	}
